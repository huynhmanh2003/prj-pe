...
public class GroupController extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if ("Add Group".equals(request.getParameter("name"))) {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        if ("Add".equals(request.getParameter("name1"))) {
            String groupname = request.getParameter("grname");
            String groupdes = request.getParameter("grdes");
            GroupDao gd1 = new GroupDao();
            gd1.addGroup(new Group(groupname, groupdes));
            request.getRequestDispatcher("ListGroup.jsp").forward(request, response);
        } else {
            request.getRequestDispatcher("ListGroup.jsp").forward(request, response);
        }
